...
private val SPLIT_GAP = 8.dp

@Composable
internal fun SplitNumberRow(
    keys: List<String>,
    onKeyPress: (String) -> Unit,
    colors: KeyboardColors,
    height: Dp = 38.dp,
    ...
    Row(
        ...
        leftKeys.forEach { key ->
            KeyButton(label = key, contentDescription = "Digit $key",
...
internal fun SplitKeyRow(
    ...
    alternativesMap: Map<String, List<String>> = KEY_ALTERNATIVES,
    height: Dp = 46.dp,
    splitAt: Int = keys.size / 2,
) {
    val leftKeys = keys.take(splitAt)
    val rightKeys = keys.drop(splitAt)
...
internal fun SplitAlphaRow3(
    ...
    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        Row(modifier = Modifier.weight(1f), horizontalArrangement = Arrangement.spacedBy(5.dp, Alignment.CenterHorizontally), verticalAlignment = Alignment.CenterVertically) {
            KeyButton(label = shiftLabel, contentDescription = shiftDesc,
                modifier = Modifier.weight(1.5f), height = height,
                bgColor = if (isCaps) MaterialTheme.colorScheme.primary else colors.keyActionBg,
                textColor = if (isCaps) MaterialTheme.colorScheme.onPrimary else colors.keyText,
                onClick = onShiftTap)
            leftKeys.forEach { key ->
...
internal fun SplitBottomRow(
    ...
    val symbolToggleDesc = if (isSymbols) "Switch to letters" else "Switch to symbols"
    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        Row(modifier = Modifier.weight(1f), horizontalArrangement = Arrangement.spacedBy(5.dp), verticalAlignment = Alignment.CenterVertically) {
            KeyButton(label = if (isSymbols) "ABC" else "?123", contentDescription = symbolToggleDesc,
                modifier = Modifier.weight(1.5f), height = height,
                bgColor = colors.keyActionBg, textColor = colors.keyText, onClick = onSymbolToggle)
            KeyButton(label = ",", contentDescription = "Comma",
                modifier = Modifier.weight(1f), height = height,
                bgColor = colors.keyBg, textColor = colors.keyText, onClick = { onKeyPress(",") })
            KeyButton(label = " ", contentDescription = "Space (left)",
                modifier = Modifier.weight(2f), height = height,
                bgColor = colors.keyBg, textColor = colors.keyText, onClick = onSpacePress)
        }
        Spacer(Modifier.width(SPLIT_GAP))
        Row(modifier = Modifier.weight(1f), horizontalArrangement = Arrangement.spacedBy(5.dp), verticalAlignment = Alignment.CenterVertically) {
            KeyButton(label = " ", contentDescription = "Space (right)",
                modifier = Modifier.weight(2f), height = height,
                bgColor = colors.keyBg, textColor = colors.keyText, onClick = onSpacePress)
            KeyButton(label = ".", contentDescription = "Period",
                modifier = Modifier.weight(1f), height = height,
                bgColor = colors.keyBg, textColor = colors.keyText, onClick = { onKeyPress(".") })
            KeyButton(label = "↵", contentDescription = returnKeyDescription,
                modifier = Modifier.weight(1.5f), height = height,
                bgColor = MaterialTheme.colorScheme.primary,
                textColor = MaterialTheme.colorScheme.onPrimary, onClick = onReturn)
        }
    }
}
